...
function shortSource(s) {
  ...
  if (t === "ahackaday") return "AH";
  if (t === "cisa-kev" || t.includes("cisa")) return "CISA";
  if (t.startsWith("rss:")) return t.replace("rss:", "").replace(/^www\./, "").slice(0, 18);
  if (t === "github-queue" || t === "queue") return "QUEUE";
  return t ? t.slice(0, 12).toUpperCase() : "SRC";
}
    ...
    blocks: [
      ...
      },
      {
        ...
        elements: [
          {
            type: "mrkdwn",
            text: "Reply with the item number + `ship`/`defer` to act quickly.",
          },
        ],
      },
    ],
