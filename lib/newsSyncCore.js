...
async function loadCandidates({ source_url, max_items, source_mode, rss_sources }, onRetry) {
  ...
  if (source_mode === "mixed") {
    const [ah, rss, kev] = await Promise.all([
      withOneRetry("ahackaday_fetch", () => fetchAhackadayItems(source_url, Math.ceil(max_items * 0.45)), onRetry),
      withOneRetry("rss_fetch", () => fetchRssBundleItems(Math.ceil(max_items * 0.35), rss_sources), onRetry),
      withOneRetry("kev_fetch", () => fetchCisaKevItems(Math.ceil(max_items * 0.2)), onRetry),
    ]);
    const merged = [...ah, ...rss, ...kev];
    const seen = new Set();
      ...
      slack = await withOneRetry(
        ...
        () =>
          postSlackFeedFromResults(results, {
            heading: `Grace — ${source_mode === "mixed" ? "Mixed Sources" : "AHackaday"} (${new Date().toISOString().slice(0, 10)})`,
          }),
        (r) => failures.retries.push(r)
      );
    } catch (e) {
