...
export async function getIncidentReport(workspaceId, incidentKey) {
  ...
  if (!extractedIndicators.length) {
    const runText = incidentRuns
      .map((r) => {
        const b = r?.run_detail?.bridge || {};
        return [b.incident_url || "", b.incident_title || "", b.incident_summary || ""].join("\n");
      })
      .join("\n");
    const fallbackSignals = extractIndicatorsFromText(runText).slice(0, 10);
    if (fallbackSignals.length) extractedIndicators.push(...fallbackSignals);
  }
