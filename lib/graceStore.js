...
export async function getIncidentReport(workspaceId, incidentKey) {
  ...
  const recommendations = recs.filter((r) => r.score_id && scoreIds.has(r.score_id));
  const latestMetric = metrics[0] || null;

  const extractedIndicators = [
    ...new Set(
      incidentRuns.flatMap((r) => {
        const indicators = r?.run_detail?.bridge?.extracted_indicators;
  ...
  return {
    ...
    kpis: {
      north_star: Number(latestMetric?.north_star_score || 0),
      answer_inclusion: Number(latestMetric?.answer_inclusion_rate || 0),
      freshness: Number(latestMetric?.content_freshness_ratio || 0),
      open_actions: recommendations.filter((r) => !["executed", "measured"].includes(r.status)).length,
    },
    north_star: Number(latestMetric?.north_star_score || 0),
    answer_inclusion: Number(latestMetric?.answer_inclusion_rate || 0),
    freshness: Number(latestMetric?.content_freshness_ratio || 0),
